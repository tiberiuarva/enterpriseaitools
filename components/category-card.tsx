    ...
    >
      ...
      <div className="mt-5 flex flex-wrap gap-2" aria-hidden="true">
        {visible.map((tool) => (
          <span
            key={tool.id}
            className="inline-flex items-center rounded-full border border-[var(--color-border)] bg-[var(--color-bg-hover)] px-2.5 py-1 text-caption text-[var(--color-text-secondary)]"
          >
            {tool.name}
          </span>
        ))}

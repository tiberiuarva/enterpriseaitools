# Logo audit report

Generated from current repo data via `npm run report-logo-audit`. No wall-clock timestamp is embedded so diffs only reflect data/reporting changes.

## Site coverage by category

| Category | Total | Fallback | Service icon | Project logo | Official product | Official vendor |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| agents | 13 | 7 (54%) | 0 | 6 | 0 | 0 |
| orchestration | 10 | 0 (0%) | 5 | 5 | 0 | 0 |
| governance | 9 | 6 (67%) | 1 | 2 | 0 | 0 |
| assistants | 11 | 8 (73%) | 2 | 1 | 0 | 0 |
| platforms | 3 | 0 (0%) | 3 | 0 | 0 | 0 |
| **All site records** | **46** | **21 (46%)** | **11** | **14** | **0** | **0** |

## Inventory status

- Inventory rows: **46**
- Classified: **46**
- Unclassified: **0**

## Highest-priority cleanup signal

- Current worst category by fallback ratio: **assistants** with **8/11** fallback entries (73%).
- Treat this report as an audit gate: do not claim the logo system is cleaned up while fallback share remains high.

